...
def httpcall(url):
    ...
    headers = {
        ...
        'Referer': f"{random.choice(headers_referers)}{buildblock(random.randint(5, 10))}",
    ...
    try:
        ...
        print(f"Request URL: {request_url}, Status Code: {response.status_code}")
    ...
    def run(self):
        while flag < 2:
            ...
            if safe == 1 and flag == 1:
                set_flag(2)
            sleep(0.1)  # Small sleep to prevent CPU overload
